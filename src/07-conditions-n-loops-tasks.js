...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i++) {
    const firstInd = str.indexOf(str[i]);
 ...
 */
function isCreditCardNumber(cardNumber) {
  ...
  const sum = digits.reduce((acc, digit) => acc + digit);

  // If the resulting sum is divisible by 10, the card number is valid
  return sum % 10 === 0;
}
...
function getDigitalRoot(num) {
  const digits = num
    ...
    .split('')
    .map((digit) => parseInt(digit, 10));
  const sum = digits.reduce((current, initial) => current + initial, 0);
